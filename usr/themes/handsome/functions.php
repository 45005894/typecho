...
<code>rgba(0,0,0,0.3)</code>，填写后你就会发现效果的，就是让透明色不太过透明，导致字都看不清了。</br></br> 一般来说，".redText('根据背景图片的主色调来填比较好看。')." </br>举例：背景图片主色调是蓝色，那就填 <code>rgba(56, 136, 255, 0.3)</code> (56,136,255) 就是蓝色，0.3 代表背景的透明度。"));
    ...
    $codeStyle = new Radio('codeStyle',
        array(
            ...
            'zenburn' => 'zenburn（深黑）',
            'custome' => '自定义'
        ),'0','代码高亮的风格选择','使用的代码高亮风格见 <a href="https://highlightjs.org/">风格列表</a>，系统默认内置 <code>vs.css</code> 和 <code>zenburn.css</code>两种。</br> 您可以在<a href="https://github.com/isagalaev/highlight.js/tree/master/src/styles">该网站</a>下载css风格文件，然后填写在 <code>assets/css/features/code/custom.min.css</code> 内，并且选择 <code>自定义</code>选项即可。');
...
{"name":"网易云音乐","img":"https://ww4.sinaimg.cn/large/a15b4afegy1fg2pouholzj201s01s0ja","value":"你的网易云账号","link":"#"}', _mt('时光机联系方式配置').' <a href="https://handsome.ihewro.com/#/customize" target="_blank"><span style="">'._mt('配置文档').'</span></a>', _mt('基本配置直接修改框中的value值和link值即可，高级配置请看文档'));
    ...
    $featuresetup = new Checkbox('featuresetup',
        array(
            ...
            'lazyload' => _t('延迟加载图片（lazyload）'),
            'sreenshot' => _t("启用文章页面截图(beta)"),
            'noShowParentCategory' => _t("当有子分类的时候不显示父分类（侧边栏的分类中显示）")
        ),
        array('musicplayer','isPjax','payforauthorinpost','laodthefont','smoothscroll','tocThree','ajaxComment','FixedImageSize','hightlightcode','lazyload'), _t('增强功能开关'),"* <code>固定头图大小比例 8:3（自动裁剪）</code>功能：只是让显示的尺寸符合8：3，实际体积大小并没有变化。</br>* <code>启用文章页面截图</code>功能：会引入160 KB的html2canvas.min.js");
...
EOF;
    ...
    //刚刚来过
    $sql_just = $db->select('COUNT(author) AS cnt', 'author', 'url', 'mail')
        ->from('table.comments')
        ->where('status = ?', 'approved')
        ->where('type = ?', 'comment')
        ->where('authorId = ?', '0')
...
function themeFields(Typecho_Widget_Helper_Layout $layout){
    ...
    $thumb = new Typecho_Widget_Helper_Form_Element_Text('thumb', NULL, NULL, _t('头图地址(thumb)'), _t('输入图片URL，则优先使用该图片作为头图，填写  <code>no</code> 则不显示该篇文章的头图。</br>不填此处则按照后台外观设置中的 <code>主题增强功能-博客头图设置</code>顺序显示头图。</br>'));
